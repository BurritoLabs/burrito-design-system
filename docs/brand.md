...
- Use exactly -1 px letter spacing for both the primary wordmark and product descriptor, as selected in the visual comparison on 2026-09-08. Apply the same tracking to Burrito Labs as well.
- The word-to-product gap is also 6 px. Use the bundled `Burrito Brand` font face (the unmodified Montserrat Latin variable font) rather than a product's own font version. Do not force text widths through SVG `textLength` or horizontal scaling, or override the shared -1 px tracking in a product.
